...
//       cy.get('.navbar-links').contains('Projects').click();
//       cy.findByTestId('project-list').should('be.visible');
//       cy.get('.navbar-links').contains('Contact').click();
...
describe('External links work as expected', () => {
   it('should navigate to LinkedIn and Github pages', () => {
      cy.visit('/');
      cy.get('.landing-page-link').click();
      cy.get('.navbar-icons').findByTestId('linkedin button').click();
      cy.url().should('include', '/');
      cy.get('.navbar-icons').findByTestId('github button').click();
      cy.url().should('include', '/');
   });
});
